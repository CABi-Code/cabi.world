...
function renderFolderItem(array $node, bool $isOwner, int $depth, \App\Repository\UserFolderRepository $folderRepository): void {
	...
	if (!$isOwner && empty($item['is_hidden']) || $isOwner):
				...
				<div class="folder-actions">
					<?php if ($isEntity): ?>
						<button class="folder-action-btn" 
								onclick="window.showCreateModal(<?= $item['id'] ?>); event.stopPropagation();" 
								title="Добавить">
							<svg width="14" height="14"><use href="#icon-plus"/></svg>
						</button>
					<?php endif; ?>
					<button class="folder-action-btn" 
							onclick="window.showSettingsModal(<?= $item['id'] ?>); event.stopPropagation();" 
							title="Настройки">
