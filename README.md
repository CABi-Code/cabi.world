...
Все доступные эндпоинты должны прописываться в файле app/Core/Security/routes.json
Поддерживается формат регулярных выражений Regex. Восклицательный знак вначале отключает маршрут для временного решения
